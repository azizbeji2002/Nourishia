...
public class EditPatientController implements Initializable {
        ...
        if (patient != null) {
            ...
            if (patient.getPlan() != null) {
                for (PlanNutritionnels plan : planComboBox.getItems()) {
                    if (plan.getIdplan().equals(patient.getPlan().getIdplan())) {
    ...
    private void handleSaveAction(ActionEvent event) {
        if (patient == null) {
            showAlert(Alert.AlertType.ERROR, "Error", "No Patient", "No patient data available for update.");
            return;
        }
        ...
        // Check if email already exists and is not the current patient's email
        ...
        try {
            ...
            showAlert(Alert.AlertType.INFORMATION, "Success", "Patient Updated", "The patient has been updated successfully.");
            ...
            showAlert(Alert.AlertType.ERROR, "Error", "Could not update the patient", e.getMessage());
        }
    }
